...
```
Сертификаты берутся из `TLS_CERTS_PATH` (по умолчанию `./docker/certs`). Если `fullchain.pem`/`privkey.pem` отсутствуют, `certgen` сгенерирует self-signed сертификат при первом запуске (можно заменить своими файлами).
